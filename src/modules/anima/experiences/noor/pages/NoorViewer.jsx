...
const CircularPlayer = ({ label, isActive, onToggle }) => {
  return (
    <div className="flex flex-col items-center gap-4">
      <div className="relative group cursor-pointer" onClick={onToggle}>
        ...
        <motion.div
          ...
          className={`w-28 h-28 rounded-full border flex items-center justify-center backdrop-blur-md shadow-2xl relative z-10 transition-colors 
                        ${
                          isActive
                            ? "border-emerald-500/60 bg-stone-900/80"
                            : "border-emerald-500/30 bg-stone-900/50 hover:border-emerald-500/60"
                        }
                    `}
        >
          {isActive ? (
      ...
      {label && (
        <span className="text-xs uppercase tracking-widest text-emerald-500/60 font-medium">
          {label}
        </span>
      )}
    </div>
...
const InlinePlayer = ({ label, isActive, onToggle }) => {
  return (
    <motion.button
      whileTap={{ scale: 0.98 }}
      onClick={onToggle}
    ...
    >
      <div
        className={`flex items-center justify-center h-10 w-10 rounded-full border ${
          isActive
            ? "bg-emerald-500 text-stone-900 border-emerald-400"
...
export default function NoorViewer({ gift: giftProp }) {
  ...
  useEffect(() => {
    ...
    if (giftProp) {
      ...
      if (giftProp?.meaningAudioUrl)
        meaningRef.current.src = giftProp.meaningAudioUrl;
      return;
    }
    ...
    const fetchGift = async () => {
      try {
        ...
        if (data?.meaningAudioUrl)
          meaningRef.current.src = data.meaningAudioUrl;
      } catch (error) {
  ...
  const toggleAudio = (type) => {
    ...
    const targetAudio =
      type === "recitation" ? recitationRef.current : meaningRef.current;
    const otherAudio =
      type === "recitation" ? meaningRef.current : recitationRef.current;
  ...
  if (!gift)
    return (
      <div className="h-screen bg-stone-950 text-white flex items-center justify-center">
        Nicht gefunden.
      </div>
    );
            ...
            >
              <ReactMarkdown
                components={{
                  p: ({ ...props }) => (
                    <p
                      className="text-xl font-light text-stone-300/90 mb-8"
              ...
              >
                {gift.meaningText ||
                  "_Keine schriftliche Bedeutung hinterlegt._"}
              </ReactMarkdown>
            </motion.div>

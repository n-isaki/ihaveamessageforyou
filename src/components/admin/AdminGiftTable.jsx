...
du hast ein persönliches Geschenk bestellt. So richtest du es ein:

1. Öffne den Link unten (der Link ist nur für dich gültig).
2. Auf der Seite: Gestalte deinen Start-Bildschirm, lade optional Fotos hoch und füge deine Nachricht oder dein Video hinzu.
3. Klicke am Ende auf „Geschenk versiegeln“. Danach starten wir die Veredelung – Änderungen sind dann nicht mehr möglich.

Dein persönlicher Link:
${setupUrl}`;
    navigator.clipboard
      .writeText(message)
      .then(() => toast.copy("Setup-Link mit Anleitung kopiert!"))
                ...
                {isSelectMode && (
                  <th className="p-2 pl-4 w-12">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        const allSelected = gifts.every((g) =>
                          selectedGifts.has(g.id)
                        );
                        gifts.forEach((g) => {
                          if (allSelected) {
                    ...
                    >
                      ...
                      <td className="p-2 align-middle">
                        <div className="flex items-center space-x-3">
                          <div
                            className={`p-2 rounded-lg shrink-0 ${exp.colors.bg} ${exp.colors.text}`}
                          >
                      ...
                      <td className="p-2 align-middle">
                        ...
                        <div
                          className="text-xs text-stone-400 truncate max-w-[150px]"
                          title={gift.customerEmail}
                        >
                          {gift.customerEmail || "-"}
                        </div>
                      </td>
                      ...
                      <td className="p-2 align-middle text-right pr-4">
                        ...
                        >
                          ...
                          <button
                            onClick={(e) => onDeleteClick(e, gift.id)}
                                      ...
                                      >
                                        ...
                                        >
                                          ...
                                          <div className="mt-2 text-center">
                                            <p className="text-[10px] font-bold text-stone-900 uppercase tracking-wide">
                                              Erstellt
                                            </p>
                                  ...
                                  <div className="mt-6 pt-4 border-t border-stone-100 flex justify-between items-center text-xs text-stone-400">
                                    <div className="flex flex-col gap-1">
                                      ...
                                      <span className="flex items-center gap-2">
                                        ID:{" "}
                                        <span className="font-mono select-all text-stone-500">
                                          {gift.id}
                                        </span>
                                        <button
                                          onClick={(e) =>
                                            handleCopyId(e, gift.id)
                                          }
                                          className="p-1 hover:bg-stone-200 rounded transition-colors"
                                          title="ID kopieren"
                                        >
                                    ...
                                    <div className="flex justify-between items-center border-b border-stone-200 pb-2">
                                      ...
                                      <dd className="font-mono font-bold flex items-center gap-2">
                                        ...
                                        {gift.accessCode && (
                                          <button
                                            onClick={(e) =>
                                              handleCopyPin(e, gift.accessCode)
                                            }
                                            className="p-1 hover:bg-stone-200 rounded transition-colors"
            ...
            >
              ...
              <div className="p-4 pl-5">
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center space-x-3">
                    <div
                      className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 ${exp.colors.bg} ${exp.colors.text}`}
                    >
                ...
                {!isSelectMode && (
                  <div className="grid grid-cols-3 gap-2 mt-4 pt-3 border-t border-stone-100">
                    <Link
                      to={`/admin/edit/${gift.id}`}
                      className="flex flex-col items-center justify-center p-2 rounded-lg bg-stone-50 text-stone-600 active:bg-stone-200"
                    >
                  ...
                  {expandedId === gift.id && (
                    ...
                    >
                      <div className="space-y-4 text-sm">
                        {gift.customerName && (
                          <div>
                            <span className="font-bold text-xs uppercase text-stone-400 block mb-1">
                              Kunde
                            </span>{" "}
                            {gift.customerName}
                          </div>
                        ...
                        <div className="pt-2">
                          <button
                            onClick={(e) => onDeleteClick(e, gift.id)}
                            className="w-full py-2 text-red-600 font-bold text-xs uppercase bg-red-50 rounded-lg"
                          >
                            Löschen
                          </button>
                        </div>

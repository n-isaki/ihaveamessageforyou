        ...
        <>
            ...
            `}>
                ...
                <nav className="flex-1 px-4 space-y-1 mt-4">
                    ...
                    {/* Gruppe 2: Produkttypen */}
                    <div className="mb-6">
                        {!collapsed && (
                            <div className="px-4 mb-3">
                                <p className="text-[10px] font-semibold text-[#8A8A8A] uppercase tracking-wider">Produkte</p>
                            </div>
                        )}
                        <Link 
                            to="/admin/dashboard?tab=kamlimos&view=list"
                            onClick={() => { onViewChange?.('list'); onClose?.(); }}
                            className={`flex items-center space-x-3 w-full px-4 py-2.5 rounded-lg transition-all font-medium text-sm ${
                                location.pathname === '/admin/dashboard' && location.search.includes('tab=kamlimos')
                                    ? 'bg-[#3A3A3A] text-white border-l-2 border-white' 
                                    : 'text-[#B0B0B0] hover:text-white hover:bg-[#353535]'
                            } ${collapsed ? 'justify-center px-2' : ''}`}
                            title="Tasse"
                        >
                            ...
                            {!collapsed && <span>Tasse</span>}
                        </Link>
                        <Link 
                            to="/admin/dashboard?tab=ritual&view=list"
                            onClick={() => { onViewChange?.('list'); onClose?.(); }}
                            className={`flex items-center space-x-3 w-full px-4 py-2.5 rounded-lg transition-all font-medium text-sm ${
                                location.pathname === '/admin/dashboard' && location.search.includes('tab=ritual')
                                    ? 'bg-[#3A3A3A] text-white border-l-2 border-white' 
                                    : 'text-[#B0B0B0] hover:text-white hover:bg-[#353535]'
                            } ${collapsed ? 'justify-center px-2' : ''}`}
                            title="Armband"
                        >
                            ...
                            {!collapsed && <span>Armband</span>}
                        </Link>
                        <Link 
                            to="/admin/dashboard?tab=memoria&view=list"
                            onClick={() => { onViewChange?.('list'); onClose?.(); }}
                            className={`flex items-center space-x-3 w-full px-4 py-2.5 rounded-lg transition-all font-medium text-sm ${
                                location.pathname === '/admin/dashboard' && location.search.includes('tab=memoria')
                                    ? 'bg-[#3A3A3A] text-white border-l-2 border-white' 
                                    : 'text-[#B0B0B0] hover:text-white hover:bg-[#353535]'
                            } ${collapsed ? 'justify-center px-2' : ''}`}
                            title="Memoria"
                        >
                            ...
                            {!collapsed && <span>Memoria</span>}
                        </Link>
                        <Link 
                            to="/admin/dashboard?tab=noor&view=list"
                            onClick={() => { onViewChange?.('list'); onClose?.(); }}
                            className={`flex items-center space-x-3 w-full px-4 py-2.5 rounded-lg transition-all font-medium text-sm ${
                                location.pathname === '/admin/dashboard' && location.search.includes('tab=noor')
                                    ? 'bg-[#3A3A3A] text-white border-l-2 border-white' 
                                    : 'text-[#B0B0B0] hover:text-white hover:bg-[#353535]'
                            } ${collapsed ? 'justify-center px-2' : ''}`}
                            title="Noor"

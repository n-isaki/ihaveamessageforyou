...
export default function AdminTaxes() {
  ...
  const fetchTaxGifts = async (showLoader = false) => {
    ...
    try {
      ...
      etsyOnly.sort((a, b) => {
        ...
        const timeA =
          getT(a.deliveredAt) || getT(a.updatedAt) || getT(a.createdAt);
        const timeB =
          getT(b.deliveredAt) || getT(b.updatedAt) || getT(b.createdAt);
        return timeB - timeA;
      });
  ...
  const handleSyncEtsyNow = async () => {
    ...
    try {
      ...
      const detail =
        error?.message?.replace(/^.*?:\s*/, "") || "Unbekannter Fehler";
  ...
  const handleDebugEtsy = async () => {
    try {
      ...
      toast.error(
        `Etsy Debug fehlgeschlagen: ${error?.message || "Unbekannter Fehler"}`,
      );
    }
  };
    ...
    setGifts((prevGifts) =>
      prevGifts.map((gift) => {
        ...
        if (platform === "etsy" && sellingPrice > 0) {
          ...
          platformFee = sellingPrice * 0.105 + 0.5;
        } else if (platform === "shopify" && sellingPrice > 0) {
          // Shopify: Approx 2.1% + 0.30€
          platformFee = sellingPrice * 0.021 + 0.3;
        }
        ...
        const incomeTaxReserve =
          profitBeforeIncomeTax > 0 ? profitBeforeIncomeTax * 0.2 : 0;
  ...
  const totalRevenue = gifts.reduce(
    (acc, g) => acc + (parseFloat(g.taxInfo?.sellingPrice) || 0),
    0,
  );
  const totalCosts = gifts.reduce(
    (acc, g) => acc + (parseFloat(g.taxInfo?.costs) || 0),
    0,
  );
  const totalEtsy = gifts.reduce(
    (acc, g) =>
      acc + (g.taxInfo?.platform === "etsy" ? g.taxInfo?.platformFee || 0 : 0),
    0,
  );
  const totalShopify = gifts.reduce(
    (acc, g) =>
      acc +
      (g.taxInfo?.platform === "shopify" ? g.taxInfo?.platformFee || 0 : 0),
    0,
  );
  const totalPlatformFees = gifts.reduce(
    (acc, g) => acc + (g.taxInfo?.platformFee || 0),
    0,
  );
  const totalFinanzamt = gifts.reduce(
    (acc, g) => acc + (g.taxInfo?.finanzamt || 0),
    0,
  );
  const totalProfit = gifts.reduce(
    (acc, g) => acc + (g.taxInfo?.profit || 0),
    0,
  );
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-stone-500 mt-1">
                  Nur Etsy-Bestellungen mit Kundendaten (E-Mail, Adresse,
                  Personalisierung).
                </p>
              </div>
            </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-start justify-between gap-2">
                          <div className="text-xs text-stone-700 break-all">
                            ...
                            <div>
                              {gift.customerEmail || "Nicht von Etsy geliefert"}
                            </div>
                          </div>
                          <button
                            onClick={() =>
                              copyToClipboard(gift.customerEmail, "E-Mail")
                            }
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="text-xs text-stone-700 whitespace-pre-line">
                            ...
                            <div>
                              {formatAddress(gift) ||
                                "Nicht von Etsy geliefert"}
                            </div>
                          </div>
                          <button
                            onClick={() =>
                              copyToClipboard(formatAddress(gift), "Adresse")
                            }
                            className="p-1.5 rounded-md text-stone-500 hover:bg-stone-100 hover:text-stone-800"
                        ...
                        <div className="text-xs text-stone-700">
                          <div className="font-semibold text-stone-900">
                            Personalisierung (Käufer)
                          </div>
                          <div>
                            {gift.personalizationText ||
                              "Keine Käufer-Personalisierung von Etsy"}
                          </div>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-3">
                        ...
                        <div className="text-xs text-stone-700 grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <div>
                              -{(gift.taxInfo.platformFee || 0).toFixed(2)}
                            </div>
                          </div>
                          <div>
                            <div className="font-semibold text-stone-900">
                            ...
                            <div>
                              -{(gift.taxInfo.finanzamt || 0).toFixed(2)}
                            </div>
                          </div>
                          <div>
                            <div className="font-semibold text-stone-900">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="text-xs text-stone-700 whitespace-pre-line">
                          ...
                          <div>
                            {formatAddress(gift) || "Nicht von Etsy geliefert"}
                          </div>
                        </div>
                        <button
                          onClick={() =>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-sm whitespace-nowrap">
                <thead className="bg-stone-100 border-b border-stone-200 text-xs font-bold uppercase text-stone-600 tracking-wider">
                  <tr>
                    <th
                      className="p-3 pl-4 w-12 text-center"
                      title="Angekommen"
                    >
                    ...
                    <th className="p-3 border-r border-stone-200 min-w-[280px]">
                      Kundendaten
                    </th>
                    <th className="p-3 border-r border-stone-200 w-32">
                      Gewerbeart
                    </th>
                    <th className="p-3 border-r border-stone-200 w-32">
                      Plattform
                    </th>
                    <th className="p-3 border-r border-stone-200 w-24">
                      Preis (€)
                    </th>
                    <th className="p-3 border-r border-stone-200 w-24">
                      Kosten (€)
                    ...
                    <th
                      className="p-3 border-r border-stone-200 w-24 text-right"
                      title="Etsy/Shopify Gebühren"
                    >
                      Gebühr (€)
                    </th>
                    <th
                      className="p-3 border-r border-stone-200 w-24 text-right"
                      title="MwSt + ca. 20% ESt-Rücklage"
                    >
                      Finanzamt (€)
                    </th>
                    <th className="p-3 border-r border-stone-200 w-24 text-right">
                      Profit (€)
                    </th>
                  ...
                  {visibleGifts.length === 0 ? (
                    ...
                    visibleGifts.map((gift) => (
                      ...
                      >
                        ...
                        {/* Customer */}
                        <td
                          className="p-3 border-r border-stone-200 align-middle truncate max-w-[200px]"
                          title={gift.customerName || gift.headline}
                        >
                        ...
                        <td className="p-3 border-r border-stone-200 align-middle text-right text-stone-500 bg-stone-50/50 font-mono text-xs">
                          {gift.taxInfo.platformFee > 0
                            ? `-${gift.taxInfo.platformFee.toFixed(2)}`
                            : "0.00"}
                        </td>

                        {/* Finanzamt (VAT + Income Tax Reserve) */}
                        <td
                          className="p-3 border-r border-stone-200 align-middle text-right text-rose-600 bg-rose-50/30 font-mono text-xs"
                          title="MwSt + 20% ESt Rücklage"
                        >
                          {gift.taxInfo.finanzamt > 0
                            ? `-${gift.taxInfo.finanzamt.toFixed(2)}`
                            : "0.00"}
                        </td>

                        {/* Computed Profit */}
                        <td
                          className={`p-3 border-r border-stone-200 align-middle text-right font-bold font-mono text-sm ${gift.taxInfo.profit > 0 ? "text-emerald-600 bg-emerald-50/50" : gift.taxInfo.profit < 0 ? "text-rose-600 bg-rose-50/50" : "text-stone-900 bg-stone-50"}`}
                        >
                          {gift.taxInfo.profit > 0 ? "+" : ""}
                          {gift.taxInfo.profit.toFixed(2)}
                        </td>
                ...
                <tfoot className="bg-stone-800 text-white font-mono text-sm tracking-wide">
                  <tr>
                    <td
                      colSpan="6"
                      className="p-4 text-right font-bold uppercase text-stone-300 font-sans text-xs"
                    >
                      Total (Summen)
                    </td>
                    ...
                    <td
                      className="p-4 border-r border-stone-700 text-right text-amber-400"
                      title={`Etsy: ${totalEtsy.toFixed(2)} | Shopify: ${totalShopify.toFixed(2)}`}
                    >
